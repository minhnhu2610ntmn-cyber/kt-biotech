...
export const Table = <T extends Record<string, any>>({
  ...
  const isIndeterminate =
    selectedRowKeys.some(key => allRowKeys.includes(key)) && !isAllSelected;
      ...
      const newSelectedRows = [
        ...dataSource.filter(record =>
          selectedRowKeys.includes(
            getRowKey(record, dataSource.indexOf(record))
          )
        ),
        ...paginatedData,
      ];
        ...
        <table className='w-full border-collapse'>
          <thead className='bg-gray-50'>
            <tr>
              {selectable && (
                <th
                  className={`${getCellClasses()} text-left border-b border-gray-200`}
                >
                  <input
                    type='checkbox'
