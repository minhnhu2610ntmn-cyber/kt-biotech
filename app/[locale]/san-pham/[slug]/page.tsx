...
export default async function ProductDetailPage({
  ...
  // Check if product is from fallback locale
  const isFallback = productData._fallbackLocale || product._fallbackLocale;
  const fallbackLocale = productData._fallbackLocale || product._fallbackLocale;
          ...
          <div className='mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-3'>
            ...
            <div>
              <Text className='font-semibold text-amber-800'>
                {locale === 'en'
                  ? 'This product content is only available in Vietnamese.'
                  : 'Nội dung sản phẩm này chỉ có sẵn bằng tiếng Anh.'}
              </Text>
              <Text className='text-amber-700 text-sm mt-1'>
                {locale === 'en'
                  ? 'We are working to translate this product. Contact us for more information.'
                  : 'Chúng tôi đang dịch nội dung này. Vui lòng liên hệ để biết thêm thông tin.'}
              </Text>
            </div>
          </div>
        )}
